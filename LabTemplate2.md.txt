...
2. `chmod u=rw,g=w,o=x banana.cabana`  
   - Means: Set the user permissions to read and write, give group write permission, and others execute permission.  
   - Assessment: The goal is to make the file readable/writable by the user, writable by the group, and executable by others.  
...
4. `chmod 751 program`  
   ...
   - Assessment: The goal is to allow the user full permissions, group limited access, and others only execution ability.  
...
3. Evaluate if `ubuntu` can add files to new user's home directory: Cannot add files because `/home/bob` is owned by bob and usually not writable by others.  
...
2. Command(s) to add `ubuntu` & user to group `squad`:  
   - `usermod -aG` command  
   - The `-a` flag means append (so existing groups are not removed).  
   - The `-G` flag specifies the target group.  

3. Command(s) to allow `squad` to view the `ubuntu` user's home directory contents:  
   - `sudo setfacl -m g:squad:rx /home/ubuntu`  

4. Command(s) to modify `share` to have group ownership of `squad`:  
   - `sudo chgrp -R squad /home/ubuntu/share`  
...
5. Describe your tests and commands with the user account:  
   ...
   - Created/edited file inside `/home/ubuntu/share` to confirm group `squad` access.  
...
4. Which account can do what actions?  
...
To add citations, provide the site and a summary of what it assisted you with.  
If generative AI was used, include which generative AI system was used and what prompt(s) you fed it.  
